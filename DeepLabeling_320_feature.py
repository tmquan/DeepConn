...
class Model(ModelDesc):
	...
	def inputs(self):
		return [
			tf.placeholder(tf.float32, (DIMZ, DIMY, DIMX, 1), 'image_p'),
			tf.placeholder(tf.float32, (DIMZ, DIMY, DIMX, 1), 'membr_p'),
			tf.placeholder(tf.float32, (DIMZ, DIMY, DIMX, 1), 'label_p'),
			]

	def build_graph(self, image_p, membr_p, label_p):
		G = tf.get_default_graph() # For round

		pi, pm, pl = image_p, membr_p, label_p #inputs
		...
		with tf.variable_scope('gen'):
			with tf.variable_scope('lbl'):
				pif, _  = self.generator(pi, last_dim=NB_FILTERS) #32
		...
		with tf.name_scope('discrim_loss'):
			discrim_if  = regDLF(toMaxLabels(pl,   factor=MAX_LABEL), pif, name='discrim_il')
			GAMMA = 1e1
			losses.append(GAMMA*discrim_if)
			add_moving_summary(discrim_if)
		...
		# Segmentation
		pz = tf.zeros_like(pi)
		viz = tf.concat([tf.concat([pi, pl, pz, pz, pz, pz, pz, pz], 2), 
						 tf.concat([pif[:,:,:,0:1], pif[:,:,:,1:2], pif[:,:,:,2:3], pif[:,:,:,3:4], pif[:,:,:,4:5], pif[:,:,:,5:6], pif[:,:,:,6:7], pif[:,:,:,7:8], ], 2), 
						 tf.concat([pif[:,:,:,8:9], pif[:,:,:,9:10], pif[:,:,:,10:11], pif[:,:,:,11:12], pif[:,:,:,12:13], pif[:,:,:,13:14], pif[:,:,:,14:15], pif[:,:,:,15:16], ], 2), 
						 tf.concat([pif[:,:,:,16:17], pif[:,:,:,17:18], pif[:,:,:,18:19], pif[:,:,:,19:20], pif[:,:,:,20:21], pif[:,:,:,21:22], pif[:,:,:,22:23], pif[:,:,:,23:24], ], 2), 
						 tf.concat([pif[:,:,:,24:25], pif[:,:,:,25:26], pif[:,:,:,26:27], pif[:,:,:,27:28], pif[:,:,:,28:29], pif[:,:,:,29:30], pif[:,:,:,30:31], pif[:,:,:,31:32], ], 2), 
						 ], 1)
